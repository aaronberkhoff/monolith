...
def test_state_methods():
    ...
    position = torch.tensor(position).view(3, 1)
    velocity = torch.tensor(velocity).view(3, 1)

    full_state_test = torch.cat([position, velocity], dim=0)

    assert torch.equal(full_state, full_state_test)
